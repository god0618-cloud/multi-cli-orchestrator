...
from mco.dispatch.execute import execute_dispatch_claude_prompt, execute_dispatch_kimi_prompt
...
def _default_kimi_sandbox_template() -> Path:
    source_tree = Path(__file__).resolve().parents[3] / "templates" / "sandbox-contracts" / "kimi-code-supervised.json"
    if source_tree.exists():
        return source_tree
    packaged = Path(__file__).resolve().parents[1] / "templates" / "sandbox-contracts" / "kimi-code-supervised.json"
    if packaged.exists():
        return packaged
    raise FileNotFoundError("kimi-code supervised sandbox template not found")
...
def smoke_kimi_code(
    ...
    created = create_task(
        config,
        "Kimi Code Adapter Smoke",
        "Run a bounded non-interactive Kimi Code adapter smoke test.",
    )
    sandbox_path = created.task_dir / "SANDBOX_CONTRACT.json"
    sandbox_path.write_text(_default_kimi_sandbox_template().read_text(encoding="utf-8"), encoding="utf-8")

    doctor = doctor_adapter("kimi-code", sandbox_path)
    if doctor.status != "READY_SUPERVISED":
        result = {
            "schema": "mco.adapter_smoke.v1.0",
            "agent": "kimi-code",
    ...
    usage_snapshot = write_usage_snapshot(created.task_dir)
    dashboard = render_dashboard(config, created.task_id)
    report_path = created.task_dir / "artifacts" / f"{dispatch['dispatch_id']}-kimi-execution-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {}

    if status == "PASS" and SMOKE_SENTINEL not in str(report.get("stdout", "")):
        status = "FAIL"
        error = "smoke sentinel missing from Kimi stdout"
